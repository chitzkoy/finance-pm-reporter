...
//        return DateTime.now().monthOfYear == month.getValue()
//    }
//
//    fun localizeMonth(month: Month, locale: String): String {
//        return month.toString()//getDisplayName(TextStyle.FULL_STANDALONE, Locale(locale.split("_")[0]))
//    }
//
...
//        val date = DateTime(year, month.getValue(), month.length(Year.isLeap(year.toLong())), 23, 59, 59)
//        return if (date.isAfterNow && !isCurrentMonth(month)) 0.0 else transactionList.balance(date)
//    }
//
//    fun income(month: Month): Double {
//        val from = DateTime(year, month.getValue(), 1, 0, 0, 0)
//        val to = DateTime(year, month.getValue(), month.length(Year.isLeap(year.toLong())), 23, 59, 59)
...
//        val to = DateTime(year, currentMonth, java.time.Month.of(currentMonth).length(Year.isLeap(year.toLong())), 23, 59, 59)
...
//            to = DateTime(currentYear, currentMonth, java.time.Month.of(currentMonth).length(Year.isLeap(year.toLong())), 23, 59, 59)
//        } else {
